# Computation and Problem Solving Aptitude
...
def arithematic_aptitude():
    driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()),options=chrome_options)
    # Step 1: Visit the website
    driver.get("https://www.indiabix.com/online-test/aptitude-test/random")
    time.sleep(3)
    ...
    for index, bix_div_container in enumerate(bix_div_containers, start=0):
        ...
        option_divs = [option.get_text().replace('\n','') for option in bix_div_container.find_all('div', class_='flex-wrap')]
        ...
        data.append(
            {
                ...
                "explaination":  explanation_list[i]["explaination"].replace('\n','')
            }
        )
    return data


# print(arithematic_aptitude())