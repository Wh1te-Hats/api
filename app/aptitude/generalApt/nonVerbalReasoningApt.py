# Visual perception, Spatial reasoning, Pattern Recognition and Problem Solving
...
def nonVerbal_aptitude():
    driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()),options=chrome_options)
    # Step 1: Visit the website
    driver.get("https://www.indiabix.com/online-test/non-verbal-reasoning-test/random")
    time.sleep(3)
    ...
    for index, bix_div_container in enumerate(bix_div_containers, start=0):
        ...
        question_text_html = bix_div_container.find('div', class_='bix-td-qtxt').get_text().replace('\xa0','')
